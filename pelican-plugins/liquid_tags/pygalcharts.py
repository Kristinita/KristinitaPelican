...
def run_pygal(data, options=[], format='svg'):
    ...
    # Config options are pretty much proxied straight through from the JSON
    # dict into the object
    config = pygal.Config()
    ...
    if chart_type == 'bar':
        chart = pygal.HorizontalBar(config) if data.get(
            'horizontal', False) else pygal.Bar(config)
